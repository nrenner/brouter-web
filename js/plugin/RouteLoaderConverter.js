...
BR.routeLoader = function(map, layersControl, routing,pois) {
	RouteLoader=L.Control.extend({
		...
		refreshTestLayer:function() {
			...
			let simplifiedLatLngs=this.getSimplifiedLatLngs();
			...
			for(let i=0;i < simplifiedLatLngs.length; i++)
			{
				// this._testLayer.addLayer(L.marker(simplifiedLatLngs[i]));
				this._testLayer.addLayer(L.circleMarker(simplifiedLatLngs[i], {
								  radius: 6,
								  fill: false,
								  color: '#FF0000',
								}));
		...
		cleanup: function(e) {
			this._testLayer.clearLayers();
			if(this._trackLayer && map.hasLayer(this._trackLayer) && (!this._options.showTrackLayer || this._closeCanceled))
			{
		...
		setSliderRange: function(){
			...
			let guessedTolerance=this.guessSimplifyTolerance(this._trackPoints);
		...
		onToleranceSlider: function(e) {
			let guess=parseFloat($(e.target).data("guess"));
			let f=parseFloat(e.target.value);
			let frac=parseFloat($(e.target).prop("max"));
			
			if(f > guess)
				this._options.simplifyTolerance=guess  +  (f ** 2) * (guess / ((frac ** 2) / 10));
			else
				this._options.simplifyTolerance=Math.abs(guess  + (f ** 3)  * (guess / (frac ** 3)));
		...
		onBusyChanged:function(isBusy=false){

			if(isBusy === true)
				$("#loadedittrackdlg #msg_busy").removeClass("invisible");
				// $("#loadedittrackdlg #busy_img").addClass("d-block");
			else
				$("#loadedittrackdlg #msg_busy").addClass("invisible");
				// $("#loadedittrackdlg #busy_img").removeClass("d-block");
		},
		...
		onAdd: function(map) {
			$('#loadedittrackdlg').on('hidden.bs.modal', (e) => {
			  this.cleanup();
			  
			}); 
			$('#loadedittrackdlg').on('show.bs.modal', (e) => { 
			...
			$('#manual_collapse').collapse('hide');
			$('#manual_collapse').on('hidden.bs.collapse shown.bs.collapse',  (e) => {this.onManualCollapse(e);});
		...
		onFileChanged: function(e) {
			...
			let testmode=this._options.isTestMode;
			this.cleanup();
		...
		getOptions: function() {
			...
			let f=$("#file_format").val();
			this._options.format=(f == "extension")?undefined:f;
		...
		convertTrackLocal: function(){
		   this.onBusyChanged(true);
			...
			if(!this._options.format)
				this._options.format=trackFile.name.split('.').pop();
		...
		getLineStringsFromGeoJSON: function(geoJSON) {
			let allLinePoints=[];
			let flat=turf.flatten(geoJSON);
		    turf.featureEach(flat,(feature,idx) => {
			   if(turf.getType(feature) == "LineString")
			   {
				    feature=turf.cleanCoords(feature);
				    let lPoints=turf.coordAll(feature);
					allLinePoints=allLinePoints.concat(lPoints);				   
			   }
			});
			
		   let linesGeoJSON=turf.lineString(allLinePoints);
		...
		guessSimplifyTolerance:function(trackPoints){
			let tolerance=(trackPoints.length  / 1000000 );			
		...
		addRoutingPoints: function (geoJSON) {
			...
			if (routingPoints.length > 0) {
			   routing.setWaypoints(routingPoints, function(event) {
				...
				if(!this._bounds)
					this._bounds =L.latLngBounds(routingPoints);
		    ...
		    if(this._options.showPointAsPoi)
			{
				turf.featureEach(this._currentGeoJSON,(feature,idx) => {
				   if(turf.getType(feature) == "Point")
				   {
					   ...
					   let coord=turf.getCoord(feature);
					   let latlng=L.GeoJSON.coordsToLatLng(coord);
					   let name="";
		...
		processFile:function(e) {
		   ...
		   if(this._options.showTrackLayer || this._options.isTestMode)
				this.addTrackOverlay(geoJSON);
